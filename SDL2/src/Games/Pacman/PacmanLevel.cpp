...
bool PacmanLevel::LoadLevel(const std::string& levelPath) {
	...
	Command bonusItemBeginCommand;
	bonusItemBeginCommand.command = "bonus_item_begin_level";
	bonusItemBeginCommand.parseFunc = [this](ParseFuncParams params) {
		mBonusItemProperties.back().begin = FileCommandLoader::ReadInt(params);
		};

	fileLoader.AddCommand(bonusItemBeginCommand);

	Command bonusItemEndCommand;
	bonusItemEndCommand.command = "bonus_item_end_level";
	bonusItemEndCommand.parseFunc = [this](ParseFuncParams params) {
		mBonusItemProperties.back().end = FileCommandLoader::ReadInt(params);
		};

	fileLoader.AddCommand(bonusItemEndCommand);
